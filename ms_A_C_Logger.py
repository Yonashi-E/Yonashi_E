...
last_event_time = time.perf_counter()  # 마지막 이벤트 시간 (밀리초 단위)
...
def analyze_times():
    ...
    # A와 D가 모두 입력되지 않았을 때 (브레이킹 후 멈춤 상태)
        ...
        if braking_started:  # 브레이킹 상태에서 둘 다 입력되지 않았을 때
            ...
            last_event_time = time.perf_counter()  # 현재 시간을 마지막 이벤트 시간으로 갱신
            log_event(f"Both 'A' and 'D' are not pressed. Inactive time: {inactive_time * 1000:.0f} ms.", GREEN, blank_line=True)  # 밀리초 출력 후 간격 추가
    ...
    try:
        if key.char.upper() in key_status:
            if key_status[key.char.upper()]:  # 키가 눌린 상태에서 해제된 경우
                ...
                log_event(f"'{key.char.upper()}' key released. Duration: {duration * 1000:.0f} ms.", RESET)  # 밀리초 출력
